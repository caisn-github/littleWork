...
#include <linux/mm.h>
#include <linux/mm_types.h> //为了适应do_anongmous_page中的 struct vm_fault *vmf这个数据结构的处理
//#include <stdio.h>
...
struct data_t {
	...
	char pid_name[TASK_COMM_LEN]; //TASK_COMM_LEN通常是16个字节
 	int type; // 用1表示是anonymous，用2表示是文件页的缺页事件	
  	unsigned long vaddr; // 虚拟地址	
};

// 探针函数，在execve系统调到入口处被触发
int trace_execve_file(struct pt_regs *ctx) {
	struct data_t data = {};
...
//	const char*filename_ptr = (const char*)PT_REGS_PARM2(ctx); 
	data.type = 2;
	// 这里有个问题，获取了参数之后, 参数是一个结构体，因此需要重新进行处理
	struct vm_fault *tmp =(struct vmfault*)PT_REGS_PARM1(ctx);
	unsigned long tmp_vaddr = 0; // 使用一个临时变量来进行偏移
	
	// 使用 bpf_proce_read_kernel来进行处理
	bpf_probe_read_kernel

	data.vaddr = tmp_vaddr;
...
// 探针函数，在execve系统调到入口处被触发
int trace_execve_anonymous(struct pt_regs *ctx) {
	struct data_t data = {};
	// 然后到内核代码中对应的位置去找到bpf对应的helpers.c的说明， /kernel/bpf/helpers.c
 	data.pid = bpf_get_current_pid_tgid() >> 32; // 因为高位存储的是PID,低32位存储的是GID 	
	//data.pid_name = bpf_get_current_pid_tgid() // 这个返回的是进程在其当前命名空间内的PID和TGID
	...
	// 经过查阅，open的函数原型是 int openat(int dirfd, const char *pathname ..), 也就是说，文件名是第二个参数，它位于rsi寄存器中。
//	const char*filename_ptr = (const char*)PT_REGS_PARM2(ctx); 
	data.vaddr = PT_REGS_PARM2(ctx);
	// 查看内核代码后，发现do_anonymous_page这个函数，它的入参是 struct vm_fault *vmf
	data.type = 1;
	// 获取第一个参数是：PT_REGS_PARM1, 获取第二个参数是：PT_REGS_PARM1
